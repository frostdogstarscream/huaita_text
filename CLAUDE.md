...
`AliOssSegmentPipeline` 分片上传至 OSS → 获取预签名 URL → 调用 `SegmentBody` API → 下载 RGBA 结果。密钥硬编码在 `ali/ali_oss_multipart_upload_util.py` 和 `ali/ali_segment_greenscreen_util.py` 中；`config.json` 仅配置 bucket/region/endpoint。
