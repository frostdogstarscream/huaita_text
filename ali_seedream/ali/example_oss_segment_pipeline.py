...
构建参数默认来自 ``ali/ali_oss_segment_pipeline_config.yaml``；也可用 ``--config`` 指定其它 YAML。
...
# 原始样例图（勿指向 output 下的 YOLO 导出图）
LOCAL_IMAGE = _ROOT / "resource" / "person_front" / "重叠人像_1500x2000.jpg"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="流水线 YAML（默认 ali/ali_oss_segment_pipeline_config.yaml）",
    )
    parser.add_argument(
        "--upload-verbose",
        action="store_true",
    ...
    pipeline = AliOssSegmentPipeline(config_path=args.config)
