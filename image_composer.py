...
from background_manager import get_background_items, resolve_person_layout, select_rotating_background
...
def build_subject_cutout(capture_path: Path, task_id: str) -> Image.Image:
    ...
    if not image.getbbox():
        raise AliSegmentError("Aliyun segment result is empty.")
    return image
...
def _place_subject_on_background(
    ...
    resized_subject = subject.resize((max(int(subject.width * scale), 1), target_height), Image.LANCZOS)
