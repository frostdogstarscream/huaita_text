...
class AliSegmentBodyGreenscreen:
    ...
    def __init__(
        ...
        # Tea 瓒呮椂鍗曚綅涓烘绉掞紱寮€鍚嚜鍔ㄩ噸璇曚互缂撹В鍋跺彂缃戠粶澶辫触
    ...
    @staticmethod
    def _is_likely_transient_ssl_or_tls(e: BaseException) -> bool:
        msg = str(e).lower()
        keys = (
            "ssl",
            "ssleof",
        ...
        *,
        ...
        for attempt in range(transient_retries):
            try:
                ...
                last = e
                if attempt + 1 >= transient_retries or not self._is_likely_transient_ssl_or_tls(e):
                    raise
                delay = 1.0 * (2**attempt)
                print(
                    f"[AliSegmentBody] SegmentBody 璇锋眰澶辫触锛堝皾璇?{attempt + 1}/{transient_retries}锛夛紝"
                    f"{delay:.1f}s 鍚庨噸璇? {e}"
                )
                time.sleep(delay)
    ...
    async def segment_oss_url_to_bgr_async(
        self,
        image_url: str,
        *,
        transient_retries: int = 4,
        ...
        for attempt in range(transient_retries):
            try:
                ...
                delay = 1.0 * (2**attempt)
                print(
                    f"[AliSegmentBody] SegmentBody 寮傛璇锋眰澶辫触锛堝皾璇?{attempt + 1}/{transient_retries}锛夛紝"
                    f"{delay:.1f}s 鍚庨噸璇? {e}"
                )
